...
build {
  ...
  provisioner "shell" {
    environment_vars = ["EXTERNAL_URL=${var.external_url}", "DEBIAN_FRONTEND=noninteractive"]
    execute_command  = "chmod +x {{ .Path }}; sudo -n env {{ .Vars }} bash '{{ .Path }}'"
    scripts          = ["${path.root}/scripts/00-preflight.sh", "${path.root}/scripts/10-install-gitlab.sh", "${path.root}/scripts/20-install-runner.sh", "${path.root}/scripts/30-install-docker.sh", "${path.root}/scripts/90-cleanup.sh"]
  }
  provisioner "shell" {
